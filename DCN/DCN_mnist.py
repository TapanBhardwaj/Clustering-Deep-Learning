...
class DCN(object):
    ...
    def initialize_model(self, ae_weights=None, gamma=0.1, optimizer='adam'):
        ...
        # self.model=load_model('results_mnist/DCN/DCN_model_mnist1120.h5')
    ...
    def clustering(self, x, y=None,
                   maxiter=7000,
                   save_dir='./results/DCN_MNIST'):
        save_interval = 70  # 5 epochs
        ...
        for ite in range(int(maxiter)):
            q, _ = self.model.predict(x, verbose=0)#data points in k-means friendly space
            ...
            if (index + 1) * self.batch_size > x.shape[0]:
                loss = self.model.train_on_batch(x=x[index * self.batch_size::],
                                                 y=[q[index * self.batch_size::], x[index * self.batch_size::]])
                index = 0
            else:
                loss = self.model.train_on_batch(x=x[index * self.batch_size:(index + 1) * self.batch_size],
                                                 y=[q[index * self.batch_size:(index + 1) * self.batch_size],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ae_weights', default='ae_weights/mnist_ae_weights.h5', help='This argument must be given')
    parser.add_argument('--save_dir', default='results/DCN_MNIST')
    ...
    t0 = time()
    y_pred = dcn.clustering(x=x, y=y, tol=args.tol, maxiter=args.maxiter, save_dir=args.save_dir)
